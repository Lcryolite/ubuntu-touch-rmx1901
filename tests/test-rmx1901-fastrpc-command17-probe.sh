...
set -e
test "$status" -eq 1
grep -Fxq 'uapi getinfo=0xc0045208 command16=0xc0205210 command17=0xc00c5211 struct_size=12' "$tmp_root/stdout"
...
grep -Fq 'struct fastrpc_ioctl_dsp_capabilities' "$source_file"
grep -Fq 'FASTRPC_IOCTL_GET_DSP_INFO' "$source_file"
grep -Fq 'query_legacy_dsp_info(fd, domain)' "$source_file"
grep -Fq 'command16 domain=%u rc=%d errno=%d attrs=' "$source_file"
