...
if ((offline == 0)); then
    ...
    run_remote camera-lifecycle runtime/camera-lifecycle.txt 'printf "dt_status="; tr -d "\\0" </proc/device-tree/soc/qcom,cam-req-mgr/status 2>&1 || :; echo; find /sys/bus/platform/devices -maxdepth 1 -mindepth 1 -type l -printf "%f\\n" | grep -Ei "cam|camera|icp" || :; for device in /sys/bus/platform/devices/*; do case "$(basename "$device")" in *cam*|*camera*|*icp*) printf "%s driver=" "$(basename "$device")"; readlink "$device/driver" 2>&1 || :;; esac; done; sudo -n dmesg | grep -Ei "CAM-(CRM|ICP)|cam_req_mgr|camera_global_pool|memory manager is not ready" || :' observed
